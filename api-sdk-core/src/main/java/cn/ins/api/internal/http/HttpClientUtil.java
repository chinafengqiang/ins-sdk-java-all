 ...
 */
package cn.ins.api.internal.http;


import cn.ins.api.FileItem;
import cn.ins.api.internal.util.InsLogger;
import cn.ins.api.internal.util.StringUtils;
import cn.ins.api.internal.util.WebUtils;
import okhttp3.*;
...
public class HttpClientUtil extends AbstractHttpClient {
    ...
    @Override
    public String doRequestBodyPost(String url, Map<String, Object> params, String charset) throws IOException {
        String ctype = "application/json;charset=" + charset;
        String query = WebUtils.buildRequestBodyQuery(params);
        byte[] content = {};
        if (query != null) {
            content = query.getBytes(charset);
        }
        return execute(METHOD_POST, ctype, content, url, charset, getConnectTimeout(), getReadTimeout(),
                getMaxIdleConnections(), getKeepAliveDuration(), getProxyHost(), getProxyPort(), getCustomHeaders(), new HashMap<>());
    }
